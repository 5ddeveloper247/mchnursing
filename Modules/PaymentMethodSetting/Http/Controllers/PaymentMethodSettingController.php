...
use Modules\RazerMS\Http\Controllers\RazerMSPlanController;
...
class PaymentMethodSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            } elseif ($method->method == 'Clover') {

                try {
                    $method_setup = PaymentMethodCredential::firstOrNew(array('lms_id' => $method->lms_id));

                    $method_setup->CLOVER_CLIENT_ID = trim($request->clover_client_id);
                    $method_setup->CLOVER_CLIENT_SECRET = trim($request->clover_client_secret);
                    $method_setup->CLOVER_CODE = trim($request->clover_code);
                    $method_setup->CLOVER_MERCHANT_ID = trim($request->clover_merchant_id);
                    $method_setup->CLOVER_EMPLOYEE_ID = trim($request->clover_employee_id);
                    $method_setup->CLOVER_ACCESS_TOKEN = trim($request->clover_access_token);
                    if ($request->clover_mode == 2) {
                        $value3 = "false";
                    } else {
                        $value3 = "true";
                    }
                    $method_setup->IS_CLOVER_LOCALHOST = $value3;
                    $method_setup->save();
                } catch (\Throwable $th) {
                    Toastr::error("Something went wrong", 'Failed');
                    return redirect()->back();
    ...
    public function testSubmit(Request $request)
    {
        ...
        }elseif ($data['method'] == "Clover") {

            $clover = new CloverController();
            $pakms = $clover->getPakmsKey();

            return view('paymentmethodsetting::clover_test', compact('pakms'));
    ...
    public function cloverTestPay(Request $request)
    {
        try {
            $clover = new CloverController();
            if($clover->makePayment($request,'test')){
                Toastr::success('Payment done successfully', 'Success');
                return redirect()->back();
            }else{
                Toastr::error('Something Went Wrong', 'Error');
