...
class VirtualClassController extends Controller
{
   ...
   if($cour1 == $cour2->total_classes ){
   ...
   }elseif($cour1 > $cour2->total_classes){
        ...
        // }
         ...
         $cour3 = DB::table('virtual_classes')->where('course_id', $request->courses)->where('type','!=','1')->get();
       $check = 1;
        ...
        //check if time maches or not 
              if($check == 2)
              {
                     Toastr::error('Please Choose Another time', trans('common.Failed'));
                        return redirect()->back();
              }
              else{
        ...
        try {
           ...
           $closeTime = Carbon::parse($request->time)->addMinutes($request->duration);
            ...
            if ($request->type == 1) {
                ...
                $coursestype = DB::table('virtual_classes')->where('course_id', $request->courses)->where('type','1')->get();
             
                $check = 1;
                 foreach( $coursestype as $courses3)

                 {
                  ...
                  $reqtime = Carbon::parse($request->time);
                       ...
                       if($startdate >= $courses3->start_date &&  $enddate <= $courses3->end_date)
                       {
                 ...
                 if($check == 2)
              {
                     ...
                     ?>
                        <script>
                        window.location.href = 'virtual-class';
                        </script>
                        <?php
                     exit();
                ...
                if (!empty($request->days)) {
                    $class->class_day = $request->class_day;
                }
